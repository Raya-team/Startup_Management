                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'product-supply-and-demands-create'}">
                    ...
                    <div class="table-responsive">
                        <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                            ...
                            <tbody>
                            <tr v-for="(pro, index) in ProductSupplyAndDemandsFilter" :id="'del_'+pro.id" class="text-center">
                                <td>
                                    <span class="text-dark-75">{{ pro.year }}</span>
                                </td>
                                <td>
                                    <span class="text-dark-75">{{ pro.general_request }}</span>
                                </td>
                                <td>
                                    <span class="text-dark-750">{{ pro.domestic_production }}</span>
                                </td>
                                <td>
                                    <span class="text-dark-750">{{ pro.importation }}</span>
                                </td>
                                <td>
                                    <span class="text-dark-750">{{ pro.unit_id.name }}</span>
                                </td>
                                <td>
                                    <router-link :to="{ name: 'product-supply-and-demands-edit', params: { id: pro.id }}">
                                        <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                            <i class="flaticon2-edit"></i>
                                        </a>
                                    </router-link>
                                    <button type="button" @click="deleteProductSupplyAndDemands(pro.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                        <i class="flaticon2-trash" :id="'icon_'+pro.id"></i>
                                    </button>
                                </td>
                                <hr>
                            </tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/product-supply-and-demands`;
                axios.get(pagi)
                    .then(response => {
                        this.productsupplyanddemands = response.data.data;
                        this.pagination = {
            ...
            deleteProductSupplyAndDemands(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/product-supply-and-demands/${id}`)
                            .then(response => {
        ...
        computed: {
            ProductSupplyAndDemandsFilter() {
                return this.productsupplyanddemands.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.year.match(this.filterText) ||
                            element.general_request.match(this.filterText) ||
                            element.domestic_production.match(this.filterText) ||
                            element.importation.match(this.filterText) ||
                            element.unit_id.name.match(this.filterText);
                    }
                })
            }
        }
