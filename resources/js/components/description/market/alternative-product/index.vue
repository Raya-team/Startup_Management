                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'alternative-products-create'}">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(data, index) in alternativeproductsFilter" :id="'del_'+data.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ data.product_title }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ data.description }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'alternative-products-edit', params: { id: data.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deletealternativeproducts(data.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <!--<i class="flaticon2-trash" :id="'icon'+data.id"></i>-->
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/alternative-products`;
                axios.get(pagi)
                    .then(response => {
                        this.alternativeproducts = response.data.data;
                        this.pagination = {
            ...
            deletealternativeproducts(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/alternative-products/${id}`)
                            .then(response => {
        ...
        computed: {
            alternativeproductsFilter() {
                return this.alternativeproducts.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.product_title.match(this.filterText) ||
                            element.description.match(this.filterText);
                    }
                })
            }
        }
