                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'product-competitors-create'}">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(data, index) in productcompetitorsFilter" :id="'del_'+data.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ data.name }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ data.geographical_region }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ data.market_share }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ data.competitive_feature }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ data.weakness }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'product-competitors-edit', params: { id: data.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteproductcompetitors(data.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/product-competitors`;
                axios.get(pagi)
                    .then(response => {
                        this.productcompetitors = response.data.data;
                        this.pagination = {
            ...
            deleteproductcompetitors(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/product-competitors/${id}`)
                            .then(response => {
        ...
        computed: {
            productcompetitorsFilter() {
                return this.productcompetitors.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.name.match(this.filterText) ||
                            element.geographical_region.match(this.filterText) ||
                            element.market_share.match(this.filterText) ||
                            element.competitive_feature.match(this.filterText) ||
                            element.weakness.match(this.filterText);
                    }
                })
            }
        }
