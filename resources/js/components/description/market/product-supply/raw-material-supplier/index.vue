        ...
        <div class="card card-custom gutter-b">
            ...
            <div class="card-header border-0 py-5">
                ...
                <div class="card-toolbar">
                    <router-link :to="{ name: 'raw-material-suppliers-create'}">
                ...
                <div class="table-responsive">
                    <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                        ...
                        <tr class="bg-gray-100 text-center">
                            <th>نام</th>
                            <th>منقطه جغرافیایی</th>
                            <th>عملیات</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr v-for="(data, index) in rawmaterialsuppliersFilter" :id="'del_'+data.id" class="text-center">
                            <td>
                            ...
                            <td>
                                <router-link :to="{ name: 'raw-material-suppliers-edit', params: { id: data.id }}">
                                    <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                        <i class="flaticon2-edit"></i>
                                    </a>
                                </router-link>
                                <button type="button" @click="deleterawmaterialsuppliers(data.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/raw-material-suppliers`;
                axios.get(pagi)
                    .then(response => {
                        this.rawmaterialsuppliers = response.data.data;
                        this.pagination = {
            ...
            deleterawmaterialsuppliers(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/raw-material-suppliers/${id}`)
                            .then(response => {
        ...
        computed: {
            rawmaterialsuppliersFilter() {
                return this.rawmaterialsuppliers.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
