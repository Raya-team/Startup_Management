                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'opportunity-points-create'}">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(data, index) in opportunitypointsFilter" :id="'del_'+data.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ data.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ data.weight }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ data.score }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'opportunity-points-edit', params: { id: data.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteopportunitypoints(data.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/opportunity-points`;
                axios.get(pagi)
                    .then(response => {
                        this.opportunitypoints = response.data.data;
                        this.pagination = {
            ...
            deleteopportunitypoints(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/opportunity-points/${id}`)
                            .then(response => {
        ...
        computed: {
            opportunitypointsFilter() {
                return this.opportunitypoints.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.description.match(this.filterText) ||
                            element.weight.match(this.filterText) ||
                            element.score.match(this.filterText);
                    }
                })
            }
        }
