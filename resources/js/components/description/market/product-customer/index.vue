                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'product-customers-create'}">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(data, index) in productcustomersFilter" :id="'del_'+data.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ data.customer_features }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ data.first_group_customers }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ data.second_group_customers }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ data.third_group_customers }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ data.fourth_group_customers }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'product-customers-edit', params: { id: data.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteproductcustomers(data.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+data.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/product-customers`;
                axios.get(pagi)
                    .then(response => {
                        this.productcustomers = response.data.data;
                        this.pagination = {
            ...
            deleteproductcustomers(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/product-customers/${id}`)
                            .then(response => {
        ...
        computed: {
            productcustomersFilter() {
                return this.productcustomers.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.customer_features.match(this.filterText) ||
                            element.first_group_customers.match(this.filterText) ||
                            element.second_group_customers.match(this.filterText) ||
                            element.third_group_customers.match(this.filterText) ||
                            element.fourth_group_customers.match(this.filterText);
                    }
                })
            }
        }
