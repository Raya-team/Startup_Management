                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'val-office-supplies-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(supply, index) in ValuationSuppliesFilter" :id="'del_'+supply.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ supply.description.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ supply.number }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ supply.owner.fname }} {{ supply.owner.lname }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ supply.unit_price }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ supply.total_price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'val-office-supplies-edit', params: { id: supply.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteSupply(supply.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+supply.id"></i>
                                        </button>
                                    </td>
                                </tr>
                                </tbody>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/val-office-supplies`;
                axios.get(pagi)
                    .then(response => {
                        this.office_supplies = response.data.data;
                        this.pagination = {
            ...
            deleteSupply(id, index) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/val-office-supplies/${id}`)
                            .then(response => {
        ...
        computed: {
            ValuationSuppliesFilter() {
                return this.office_supplies.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
