                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        <h3 class="card-title align-items-start flex-column">
                            <span class="card-label font-weight-bolder text-dark">هزینه‌های قبل بهره‌برداری</span>
                        </h3>
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'val-pre-operating-costs-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(cost, index) in ValuationCostsFilter" :id="'del_'+cost.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ cost.description.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ cost.owner.fname }} {{ cost.owner.lname }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ cost.total_price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'val-pre-operating-costs-edit', params: { id: cost.id }}">
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/val-pre-operating-costs`;
                axios.get(pagi)
                    .then(response => {
                        this.pre_operation_costs = response.data.data;
                        this.pagination = {
            ...
            deleteCost(id, index) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/val-pre-operating-costs/${id}`)
                            .then(response => {
        ...
        computed: {
            ValuationCostsFilter() {
                return this.pre_operation_costs.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
