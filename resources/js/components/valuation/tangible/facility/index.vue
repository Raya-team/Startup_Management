                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        <h3 class="card-title align-items-start flex-column">
                            <span class="card-label font-weight-bolder text-dark">تاسیسات</span>
                        </h3>
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'val-facilities-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(facility, index) in ValuationFacilitiesFilter" :id="'del_'+facility.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ facility.description.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ facility.number }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ facility.owner.fname }} {{ facility.owner.lname }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ facility.unit_price }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ facility.total_price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'val-facilities-edit', params: { id: facility.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteFacility(facility.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+facility.id"></i>
                                        </button>
                                    </td>
                                </tr>
                                </tbody>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteFacility(id, index) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/val-facilities/${id}`)
                            .then(response => {
        ...
        computed: {
            ValuationFacilitiesFilter() {
                return this.facilities.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
