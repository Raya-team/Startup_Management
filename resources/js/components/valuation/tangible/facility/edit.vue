...
<script>
    ...
    export default {
        ...
        created() {
            axios.get(`/api/val-facilities/${this.$route.params.id}/edit`)
                .then(response => {
                    console.log(response);
                    this.shareholders = response.data.shareholders;
                    this.facilities = response.data.facilities;
                    this.data.valuation_facilities[0] = response.data.valuation_facilities;
                })
        ...
        methods: {
            onSubmit() {
                ...
                axios.put(`/val-facilities/${this.$route.params.id}`, this.data)
                    .then(response => {
