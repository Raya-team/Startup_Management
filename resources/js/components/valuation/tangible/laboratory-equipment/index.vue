                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        <h3 class="card-title align-items-start flex-column">
                            <span class="card-label font-weight-bolder text-dark">تجهیزات آزمایشگاهی</span>
                        </h3>
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'val-laboratory-equipments-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(laboratory, index) in ValuationLaboratoriesFilter" :id="'del_'+laboratory.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ laboratory.description.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ laboratory.number }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ laboratory.owner.fname }} {{ laboratory.owner.lname }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ laboratory.dollar_unit_price }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ laboratory.toman_unit_price }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ laboratory.total_price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'val-laboratory-equipments-edit', params: { id: laboratory.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteLaboratory(laboratory.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+laboratory.id"></i>
                                        </button>
                                    </td>
                                </tr>
                                </tbody>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/val-laboratory-equipments`;
                axios.get(pagi)
                    .then(response => {
                        this.laboratory_equipments = response.data.data;
                        this.pagination = {
            ...
            deleteLaboratory(id, index) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/val-laboratory-equipments/${id}`)
                            .then(response => {
        ...
        computed: {
            ValuationLaboratoriesFilter() {
                return this.laboratory_equipments.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        let FullName = element.owner.fname + ' ' + element.owner.lname;

                        return element.description.description.match(this.filterText) ||
                            element.number.toString().match(this.filterText) ||
                            element.owner.fname.match(this.filterText) ||
                            element.owner.lname.match(this.filterText) ||
                            FullName.match(this.filterText) ||
