                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        <h3 class="card-title align-items-start flex-column">
                            <span class="card-label font-weight-bolder text-dark">تجهییزات و ماشین آلات فنی</span>
                        </h3>
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'val-equipment-and-machineries-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(machinery, index) in ValuationMachineriesFilter" :id="'del_'+machinery.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ machinery.description.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ machinery.number }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ machinery.owner.fname }} {{ machinery.owner.lname }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ machinery.unit_price }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ machinery.total_price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'val-equipment-and-machineries-edit', params: { id: machinery.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteMachinery(machinery.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+machinery.id"></i>
                                        </button>
                                    </td>
                                </tr>
                                </tbody>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/val-equipment-and-machineries`;
                axios.get(pagi)
                    .then(response => {
                        this.machineries = response.data.data;
                        this.pagination = {
            ...
            deleteMachinery(id, index) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/val-equipment-and-machineries/${id}`)
                            .then(response => {
        ...
        computed: {
            ValuationMachineriesFilter() {
                return this.machineries.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
