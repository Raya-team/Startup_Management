                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'val-tenements-create' }">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tr v-for="(tenement, index) in ValuationTenementsFilter" :id="'del_'+tenement.id" class="text-center">
                                    ...
                                    <td>
                                        <span class="text-dark-750">{{ tenement.total_price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'val-tenements-edit', params: { id: tenement.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteTenement(tenement.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+tenement.id"></i>
                                        </button>
                                    </td>
                                </tr>
                                </tbody>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/val-tenements`;
                axios.get(pagi)
                    .then(response => {
                        this.tenements = response.data.data;
                        this.pagination = {
        ...
        computed: {
            ValuationTenementsFilter() {
                return this.tenements.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
