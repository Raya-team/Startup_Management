        ...
        <div class="card card-custom gutter-b">
            ...
            <div class="card-header border-0 py-5">
                <h3 data-v-794e3638="" class="card-title align-items-start flex-column">
                    <span data-v-794e3638="" class="card-label font-weight-bolder text-dark">تامین از سرمایه گذران جدید</span>
                </h3>
                ...
                <div class="card-toolbar">
                    <router-link :to="{ name: 'new-investors-create'}">
            ...
            <div class="card-body pt-0 pb-3">
                ...
                <div class="table-responsive">
                    <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                        ...
                        <tbody>
                        <tr v-for="(data, index) in NewInvestorsFilter" :id="'del_'+data.id" class="text-center">
                            <td>
                                <span class="text-dark-75">{{ data.supplier }}</span>
                            </td>
                            <td>
                                <span class="text-dark-75">{{ data.investment }}</span>
                            </td>
                            <td>
                                <router-link :to="{ name: 'new-investors-edit', params: { id: data.id }}">
                                    <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                        <i class="flaticon2-edit"></i>
                                    </a>
                                </router-link>
                                <button type="button" @click="deleteNewInvestor(data.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/new-investors`;
                axios.get(pagi)
                    .then(response => {
                        this.new_investors = response.data.data;
                        this.pagination = {
        ...
        computed: {
            NewInvestorsFilter() {
                return this.new_investors.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.supplier.match(this.filterText) ||
