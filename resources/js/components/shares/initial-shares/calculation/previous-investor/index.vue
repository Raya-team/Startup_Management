                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'previousinvestors-create' }">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tr v-for="(previousinvestor, index) in previousinvestorsFilter" :id="'del_'+previousinvestor.id" class="text-center">
                                    ...
                                    <td>
                                        <span class="text-dark-75">{{ previousinvestor.name }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ previousinvestor.percent }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'previousinvestors-edit', params: { id: previousinvestor.id }}">
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || '/api/calculations';
                axios.get(pagi)
                    .then(response => {
                        this.previousinvestors = response.data.previousinvestors.data;
                        this.pagination = {
                            current_page: response.data.previousinvestors.current_page,
                            last_page: response.data.previousinvestors.last_page,
                            from_page: response.data.previousinvestors.from,
                            to_page: response.data.previousinvestors.to,
                            total_page: response.data.previousinvestors.total,
                            path_page: response.data.previousinvestors.path+"?page=",
                            links: response.data.previousinvestors.links,
                            first_link: response.data.previousinvestors.first_page_url,
                            last_link: response.data.previousinvestors.last_page_url,
                            prev_link: response.data.previousinvestors.prev_page_url,
                            next_link: response.data.previousinvestors.next_page_url,
                        };
                        this.progress = false;
                    });
            },
        ...
        computed: {
            previousinvestorsFilter() {
                return this.previousinvestors.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
