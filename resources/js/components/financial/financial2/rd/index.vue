                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'rds-create', params: { year: this.$route.params.id }}">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(rd, index) in rdsFilter" :id="'del_'+rd.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ ++index }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ rd.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ rd.annual_cost }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'rds-edit', params: { id: rd.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteRD(rd.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+rd.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/rds/${this.$route.params.id}`;
                axios.get(pagi)
                    .then(response => {
                        this.rds = response.data.data;
                        this.pagination = {
        ...
        computed: {
            rdsFilter() {
                return this.rds.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
