                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'manpowers-create', params: { year: this.$route.params.id }}">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tr v-for="(manpower, index) in manpowersFilter" :id="'del_'+manpower.id" class="text-center">
                                    ...
                                    <td>
                                        <span class="text-dark-750">{{ manpower.salary }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ manpower.total_rights }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'manpowers-edit', params: { id: manpower.id }}">
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/manpowers/${this.$route.params.id}`;
                axios.get(pagi)
                    .then(response => {
                        this.manpowers = response.data.data;
                        this.pagination = {
        ...
        computed: {
            manpowersFilter() {
                return this.manpowers.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.description.match(this.filterText) ||
                            element.number.toString().match(this.filterText) ||
                            element.salary.toString().match(this.filterText) ||
                            element.total_rights.toString().match(this.filterText);
                    }
                })
            }
        }
