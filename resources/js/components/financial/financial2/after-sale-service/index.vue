                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                                <router-link :to="{ name: 'aftersaleservices-create', params: { year: this.$route.params.id }}">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(aftersaleservice, index) in aftersaleservicesFilter" :id="'del_'+aftersaleservice.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ ++index }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ aftersaleservice.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ aftersaleservice.number }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ aftersaleservice.unit_cost }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ aftersaleservice.total_cost }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'aftersaleservices-edit', params: { id: aftersaleservice.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteAfterSaleService(aftersaleservice.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+aftersaleservice.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/aftersaleservices/${this.$route.params.id}`;
                axios.get(pagi)
                    .then(response => {
                        this.aftersaleservices = response.data.data;
                        this.pagination = {
            ...
            deleteAfterSaleService(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/aftersaleservices/${id}`)
                            .then(response => {
        ...
        computed: {
            aftersaleservicesFilter() {
                return this.aftersaleservices.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
