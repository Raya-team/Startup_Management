...
<script>
    ...
    export default {
        ...
        created() {
            this.Auth.check();
            axios.get(`/api/developmentcosts/${this.$route.params.id}/edit`)
                .then(response => {
        ...
        methods: {
            onSubmit() {
                ...
                axios.put(`/developmentcosts/${this.$route.params.id}`, this.data)
                    .then(response => {
            ...
            goBack() {
                this.$router.push({path: `/financial2/year/${this.data.year}`});
                setTimeout(() => {
                    var someTabTriggerEl = document.querySelector('#development-tab');
