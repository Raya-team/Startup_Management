                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'transportationcosts-create', params: { year: this.$route.params.id }}">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(transportationcost, index) in transportationcostsFilter" :id="'del_'+transportationcost.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ ++index }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ transportationcost.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ transportationcost.number }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ transportationcost.unit_cost }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ transportationcost.total_cost }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'transportationcosts-edit', params: { id: transportationcost.id }}">
                                            <a class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteTransportationCost(transportationcost.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+transportationcost.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                pagi = pagi || `/api/transportationcosts/${this.$route.params.id}`;
                axios.get(pagi)
                    .then(response => {
                        this.transportationcosts = response.data.data;
                        this.pagination = {
            ...
            deleteTransportationCost(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/transportationcosts/${id}`)
                            .then(response => {
        ...
        computed: {
            transportationcostsFilter() {
                return this.transportationcosts.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.description.match(this.filterText) ||
                            ...
                            element.total_cost.toString().match(this.filterText);
                    }
                })
            }
        }
