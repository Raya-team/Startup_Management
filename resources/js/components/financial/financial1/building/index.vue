                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'buildings-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(building, index) in buildingsFilter" :id="'del'+building.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ ++index }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ building.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ building.area }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ building.price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'buildings-edit', params: { id: building.id }}">
                                            <a href="#" class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deletebuilding(building.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon'+building.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                ...
                axios.get(pagi)
                    .then(response => {
                        this.buildings = response.data.buildings.data;
                        this.pagination = {
                            current_page: response.data.buildings.current_page,
                            last_page: response.data.buildings.last_page,
                            from_page: response.data.buildings.from,
                            to_page: response.data.buildings.to,
                            total_page: response.data.buildings.total,
                            path_page: response.data.buildings.path+"?page=",
                            links: response.data.buildings.links,
                            first_link: response.data.buildings.first_page_url,
                            last_link: response.data.buildings.last_page_url,
                            prev_link: response.data.buildings.prev_page_url,
                            next_link: response.data.buildings.next_page_url,
                        };
                        this.progress = false;
                    });
            },
            deletebuilding(id, index) {
                this.Auth.check();
        ...
        computed: {
            buildingsFilter() {
                return this.buildings.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.description.match(this.filterText) ||
                            element.area.toString().match(this.filterText) ||
                            element.price.toString().match(this.filterText);
                    }
                })
            }
        }
