                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'preoperatingcosts-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(preoperatingcost, index) in preoperatingcostsFilter" :id="'del_'+preoperatingcost.id" class="text-center">
                                    <td>
                                    ...
                                    <td>
                                        <router-link :to="{ name: 'preoperatingcosts-edit', params: { id: preoperatingcost.id }}">
                                            <a href="#" class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deletepreoperatingcost(preoperatingcost.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+preoperatingcost.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                ...
                axios.get(pagi)
                    .then(response => {
                        this.preoperatingcosts = response.data.preoperatingcosts.data;
                        this.pagination = {
                            current_page: response.data.preoperatingcosts.current_page,
                            last_page: response.data.preoperatingcosts.last_page,
                            from_page: response.data.preoperatingcosts.from,
                            to_page: response.data.preoperatingcosts.to,
                            total_page: response.data.preoperatingcosts.total,
                            path_page: response.data.preoperatingcosts.path+"?page=",
                            links: response.data.preoperatingcosts.links,
                            first_link: response.data.preoperatingcosts.first_page_url,
                            last_link: response.data.preoperatingcosts.last_page_url,
                            prev_link: response.data.preoperatingcosts.prev_page_url,
                            next_link: response.data.preoperatingcosts.next_page_url,
                        };
                        this.progress = false;
                    });
            },
            deletepreoperatingcost(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/preoperatingcosts/${id}`)
                            .then(response => {
        ...
        computed: {
            preoperatingcostsFilter() {
                return this.preoperatingcosts.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
