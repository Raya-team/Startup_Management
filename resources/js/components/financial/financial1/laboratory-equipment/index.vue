                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'laboratory-equipments-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(laboratory_equipment, index) in LaboratoryEquipmentsFilter" :id="'del_'+laboratory_equipment.id" class="text-center">
                                    <td>
                                    ...
                                    <td>
                                        <span class="text-dark-750">{{ laboratory_equipment.dollar_unit_price }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ laboratory_equipment.toman_unit_price }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-750">{{ laboratory_equipment.total_price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'laboratory-equipments-edit', params: { id: laboratory_equipment.id }}">
                                            <a href="#" class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteLaboratoryEquipment(laboratory_equipment.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+laboratory_equipment.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                ...
                axios.get(pagi)
                    .then(response => {
                        this.laboratory_equipments = response.data.laboratory_equipments.data;
                        this.pagination = {
                            current_page: response.data.laboratory_equipments.current_page,
                            last_page: response.data.laboratory_equipments.last_page,
                            from_page: response.data.laboratory_equipments.from,
                            to_page: response.data.laboratory_equipments.to,
                            total_page: response.data.laboratory_equipments.total,
                            path_page: response.data.laboratory_equipments.path+"?page=",
                            links: response.data.laboratory_equipments.links,
                            first_link: response.data.laboratory_equipments.first_page_url,
                            last_link: response.data.laboratory_equipments.last_page_url,
                            prev_link: response.data.laboratory_equipments.prev_page_url,
                            next_link: response.data.laboratory_equipments.next_page_url,
                        };
                        this.progress = false;
                    });
            },
            deleteLaboratoryEquipment(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/laboratory-equipments/${id}`)
                            .then(response => {
        ...
        computed: {
            LaboratoryEquipmentsFilter() {
                return this.laboratory_equipments.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
                        return element.description.match(this.filterText) ||
                            element.number.toString().match(this.filterText) ||
                            element.dollar_unit_price.toString().match(this.filterText) ||
                            element.toman_unit_price.toString().match(this.filterText) ||
