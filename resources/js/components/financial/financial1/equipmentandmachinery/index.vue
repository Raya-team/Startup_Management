                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'equipmentandmachineries-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(equipmentandmachinery, index) in equipmentandmachineriesFilter" :id="'del_'+equipmentandmachinery.id" class="text-center">
                                    <td>
                                        <span class="text-dark-75">{{ ++index }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ equipmentandmachinery.description }}</span>
                                    </td>
                                    <td>
                                        <span class="text-dark-75">{{ equipmentandmachinery.count }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <router-link :to="{ name: 'equipmentandmachineries-edit', params: { id: equipmentandmachinery.id }}">
                                            <a href="#" class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deleteequipmentandmachinery(equipmentandmachinery.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+equipmentandmachinery.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                ...
                axios.get(pagi)
                    .then(response => {
                        this.equipmentandmachineries = response.data.equipmentandmachineries.data;
                        this.pagination = {
                            current_page: response.data.equipmentandmachineries.current_page,
                            last_page: response.data.equipmentandmachineries.last_page,
                            from_page: response.data.equipmentandmachineries.from,
                            to_page: response.data.equipmentandmachineries.to,
                            total_page: response.data.equipmentandmachineries.total,
                            path_page: response.data.equipmentandmachineries.path+"?page=",
                            links: response.data.equipmentandmachineries.links,
                            first_link: response.data.equipmentandmachineries.first_page_url,
                            last_link: response.data.equipmentandmachineries.last_page_url,
                            prev_link: response.data.equipmentandmachineries.prev_page_url,
                            next_link: response.data.equipmentandmachineries.next_page_url,
                        };
                        this.progress = false;
                    });
            },
            deleteequipmentandmachinery(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/equipmentandmachineries/${id}`)
                            .then(response => {
        ...
        computed: {
            equipmentandmachineriesFilter() {
                return this.equipmentandmachineries.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
