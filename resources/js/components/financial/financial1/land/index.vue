                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tr v-for="(land, index) in LandsFilter" :id="'del_'+land.id" class="text-center">
                                    ...
                                    <td>
                                        <span class="text-dark-750">{{ land.price }}</span>
                                    </td>
                                    <td>
                                        <router-link :to="{ name: 'land-edit', params: { id: land.id }}">
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                ...
                axios.get(pagi)
                    .then(response => {
                        this.lands = response.data.lands.data;
                        this.pagination = {
                            current_page: response.data.lands.current_page,
                            last_page: response.data.lands.last_page,
                            from_page: response.data.lands.from,
                            to_page: response.data.lands.to,
                            total_page: response.data.lands.total,
                            path_page: response.data.lands.path+"?page=",
                            links: response.data.lands.links,
                            first_link: response.data.lands.first_page_url,
                            last_link: response.data.lands.last_page_url,
                            prev_link: response.data.lands.prev_page_url,
                            next_link: response.data.lands.next_page_url,
                        };
                        this.progress = false;
                    });
            },
        ...
        computed: {
            LandsFilter() {
                return this.lands.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
