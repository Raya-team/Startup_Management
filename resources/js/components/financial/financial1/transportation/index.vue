                ...
                <div class="card card-custom gutter-b">
                    ...
                    <div class="card-header border-0 py-5">
                        ...
                        <div class="card-toolbar">
                            <router-link :to="{ name: 'transportations-create' }">
                    ...
                    <div class="card-body pt-0 pb-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-vertical-center table-borderless" id="myTable">
                                ...
                                <tbody>
                                <tr v-for="(transportation, index) in transportationsFilter" :id="'del_'+transportation.id" class="text-center">
                                    <td>
                                    ...
                                    <td>
                                        <router-link :to="{ name: 'transportations-edit', params: { id: transportation.id }}">
                                            <a href="#" class="btn btn-icon btn-light-success btn-sm mr-2">
                                                <i class="flaticon2-edit"></i>
                                            </a>
                                        </router-link>
                                        <button type="button" @click="deletetransportation(transportation.id, index)"  class="btn btn-icon btn-light-danger btn-sm mr-2">
                                            <i class="flaticon2-trash" :id="'icon_'+transportation.id"></i>
                                        </button>
                                    </td>
                                    <hr>
                                </tr>
    ...
    export default {
        ...
        methods: {
            getResults(pagi) {
                ...
                axios.get(pagi)
                    .then(response => {
                        this.transportations = response.data.transportations.data;
                        this.pagination = {
                            current_page: response.data.transportations.current_page,
                            last_page: response.data.transportations.last_page,
                            from_page: response.data.transportations.from,
                            to_page: response.data.transportations.to,
                            total_page: response.data.transportations.total,
                            path_page: response.data.transportations.path+"?page=",
                            links: response.data.transportations.links,
                            first_link: response.data.transportations.first_page_url,
                            last_link: response.data.transportations.last_page_url,
                            prev_link: response.data.transportations.prev_page_url,
                            next_link: response.data.transportations.next_page_url,
                        };
                        this.progress = false;
                    });
            },
            deletetransportation(id, index) {
                this.Auth.check();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/transportations/${id}`)
                            .then(response => {
        ...
        computed: {
            transportationsFilter() {
                return this.transportations.filter((element) => {
                    if(!this.filterText){
                        return element
                    }else{
